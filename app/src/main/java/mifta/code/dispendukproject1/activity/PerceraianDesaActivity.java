...
public class PerceraianDesaActivity extends AppCompatActivity {
    TextView tanggal, bulan, tahun, hari, total_kab, nama, judul;
    RecyclerView recyclerView;
    ProgressBar progressBar;
    private List<tampil> results = new ArrayList<>();
    private PerceraianDesaAdapter perceraianDesaAdapter;
    private int no_kec;
        ...
        aksi.enqueue(new Callback<respon>() {
            ...
            public void onResponse(Call<respon> call, Response<respon> response) {
                ...
                if (response.code() != 200) {
                    Toast.makeText(PerceraianDesaActivity.this, "Token tidak valid atau Token expired", Toast.LENGTH_SHORT).show();
                    logout();
                } else {
                    showLoading(false);
                    results = response.body().getResult();
                    perceraianDesaAdapter = new PerceraianDesaAdapter(PerceraianDesaActivity.this, results);
                    recyclerView.getRecycledViewPool().clear();
                    perceraianDesaAdapter.notifyDataSetChanged();
                    recyclerView.setAdapter(perceraianDesaAdapter);
                }
            }
    ...
    private void logout() {
        ...
        akses.clear();
        akses.commit();
        startActivity(new Intent(PerceraianDesaActivity.this, LoginActivity.class));
        finish();
    }
}