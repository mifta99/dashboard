...
public class PerekamanDesaActivity extends AppCompatActivity {
    TextView tanggal, bulan, tahun, hari, total_kab, nama, judul;
    RecyclerView recyclerView;
    ProgressBar progressBar;
    private List<tampil> results = new ArrayList<>();
    private PerekamanDesaAdapter perekamanDesaAdapter;
    private int no_kec;
    ...
    private void desa() {
        ...
        Call<respon> aksi = api.perekaman_desa(jwt_, no_kec);
        ;
        ...
        aksi.enqueue(new Callback<respon>() {
            ...
            public void onResponse(Call<respon> call, Response<respon> response) {
                ...
                if (response.code() != 200) {
                    Toast.makeText(PerekamanDesaActivity.this, "Token tidak valid atau Token expired", Toast.LENGTH_SHORT).show();
                    logout();
                } else {
                    showLoading(false);
                    results = response.body().getResult();
                    perekamanDesaAdapter = new PerekamanDesaAdapter(PerekamanDesaActivity.this, results);
                    recyclerView.getRecycledViewPool().clear();
                    perekamanDesaAdapter.notifyDataSetChanged();
                    recyclerView.setAdapter(perekamanDesaAdapter);
                }
            }
    ...
    private void logout() {
        ...
        akses.clear();
        akses.commit();
        startActivity(new Intent(PerekamanDesaActivity.this, LoginActivity.class));
        finish();
    }
}