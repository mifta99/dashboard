...
import mifta.code.dispendukproject1.adapter.KematianDesaAdapter;
...
public class KematianDesaActivity extends AppCompatActivity {
    TextView tanggal, bulan, tahun, hari, total_kab, nama, judul;
    RecyclerView recyclerView;
    ProgressBar progressBar;
    private List<tampil> results = new ArrayList<>();
    private KematianDesaAdapter kematianDesaAdapter;
    private int no_kec;
    ...
    private void desa() {
        ...
        Call<respon> aksi = api.kematian_desa(jwt_, no_kec);
        ;
        ...
        aksi.enqueue(new Callback<respon>() {
            ...
            public void onResponse(Call<respon> call, Response<respon> response) {
                ...
                if (response.code() != 200) {
                    Toast.makeText(KematianDesaActivity.this, "Token tidak valid atau Token expired", Toast.LENGTH_SHORT).show();
                    logout();
                } else {
                    showLoading(false);
                    results = response.body().getResult();
                    kematianDesaAdapter = new KematianDesaAdapter(KematianDesaActivity.this, results);
                    recyclerView.getRecycledViewPool().clear();
                    kematianDesaAdapter.notifyDataSetChanged();
                    recyclerView.setAdapter(kematianDesaAdapter);
                }
            }
    ...
    private void logout() {
        ...
        akses.clear();
        akses.commit();
        startActivity(new Intent(KematianDesaActivity.this, LoginActivity.class));
        finish();
    }
}