...
import mifta.code.dispendukproject1.adapter.PerekamanDesaAdapter;
import mifta.code.dispendukproject1.adapter.PerkawinanDesaAdapter;
...
public class PerkawinanDesaActivity extends AppCompatActivity {
    TextView tanggal, bulan, tahun, hari, total_kab, nama, judul;
    RecyclerView recyclerView;
    ProgressBar progressBar;
    private List<tampil> results = new ArrayList<>();
    private PerkawinanDesaAdapter perkawinanDesaAdapter;
    private int no_kec;
    ...
    private void desa() {
        ...
        Call<respon> aksi = api.perkawinan_desa(jwt_, no_kec);;
        ...
        aksi.enqueue(new Callback<respon>() {
            ...
            public void onResponse(Call<respon> call, Response<respon> response) {
                ...
                if (response.code() != 200) {
                    Toast.makeText(PerkawinanDesaActivity.this, "Token tidak valid atau Token expired", Toast.LENGTH_SHORT).show();
                    logout();
                } else {
                    showLoading(false);
                    results = response.body().getResult();
                    perkawinanDesaAdapter = new PerkawinanDesaAdapter(PerkawinanDesaActivity.this, results);
                    recyclerView.getRecycledViewPool().clear();
                    perkawinanDesaAdapter.notifyDataSetChanged();
                    recyclerView.setAdapter(perkawinanDesaAdapter);
                }
            }
    ...
    private void logout() {
        ...
        akses.clear();
        akses.commit();
        startActivity(new Intent(PerkawinanDesaActivity.this, LoginActivity.class));
        finish();
    }
}