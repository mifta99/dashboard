...
import mifta.code.dispendukproject1.adapter.PencetakanDesaAdapter;
...
public class PencetakanDesaActivity extends AppCompatActivity {
    TextView tanggal, bulan, tahun, hari, total_kab, nama, judul;
    RecyclerView recyclerView;
    ProgressBar progressBar;
    private List<tampil> results = new ArrayList<>();
    private PencetakanDesaAdapter pencetakanDesaAdapter;
    private int no_kec;
    ...
    private void desa() {
        ...
        Call<respon> aksi = api.pencetakan_desa(jwt_, no_kec);
        ;
        ...
        aksi.enqueue(new Callback<respon>() {
            ...
            public void onResponse(Call<respon> call, Response<respon> response) {
                ...
                if (response.code() != 200) {
                    Toast.makeText(PencetakanDesaActivity.this, "Token tidak valid atau Token expired", Toast.LENGTH_SHORT).show();
                    logout();
                } else {
                    showLoading(false);
                    results = response.body().getResult();
                    pencetakanDesaAdapter = new PencetakanDesaAdapter(PencetakanDesaActivity.this, results);
                    recyclerView.getRecycledViewPool().clear();
                    pencetakanDesaAdapter.notifyDataSetChanged();
                    recyclerView.setAdapter(pencetakanDesaAdapter);
                }
            }
    ...
    private void logout() {
        ...
        akses.clear();
        akses.commit();
        startActivity(new Intent(PencetakanDesaActivity.this, LoginActivity.class));
        finish();
    }
}